...
class GhostwriterDocxTemplate(DocxTemplate):
    ...
    def _strip_excel_table_tags(self, xml: str) -> str:
        ...
        def _normalise_statement(match: re.Match[str]) -> str:
            statement = match.group(0)
            statement = open_tr_pattern.sub(_replace_open_tr, statement)
            statement = close_tr_pattern.sub(_replace_close_tr, statement)
            statement = tc_pattern.sub(_strip_tc, statement)
            return statement

        xml = _JINJA_STATEMENT_RE.sub(_normalise_statement, xml)
