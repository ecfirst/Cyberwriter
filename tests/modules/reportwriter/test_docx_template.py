...
CHART_TC_SERIES_XML = (
    ...
    "<c:cat><c:strLit>"
    "{%tc for domain in domains %}"
    '<c:pt idx="{{ loop.index0 }}"><c:v>{{ domain.name }}</c:v></c:pt>'
    "{%tc endfor %}"
    "</c:strLit></c:cat>"
    "<c:val><c:numLit>"
    "{%tc for domain in domains %}"
    '<c:pt idx="{{ loop.index0 }}"><c:v>{{ domain.total }}</c:v></c:pt>'
    "{%tc endfor %}"
    "</c:numLit></c:val>"
...
def test_patch_xml_renders_tc_chart_series_cleanly():
    template = GhostwriterDocxTemplate("DOCS/sample_reports/template.docx")

    cleaned = template.patch_xml(CHART_TC_SERIES_XML)

    assert "{%tc" not in cleaned
    rendered = Environment().from_string(cleaned).render(
        domains=[{"name": "corp.example.com", "total": 7}, {"name": "eng.example.com", "total": 3}]
    )
