...
def test_patch_xml_unwraps_tc_cells_with_inline_strings():
    ...
    assert '<c r="B1"' not in cleaned
    assert '<c r="C1"' not in cleaned
    assert '<c r="A1" t="inlineStr"><is><t>{% for domain in domains %}</t></is></c>' not in cleaned
    assert '<c r="C1" t="inlineStr"><is><t>{% endfor %}</t></is></c>' not in cleaned
